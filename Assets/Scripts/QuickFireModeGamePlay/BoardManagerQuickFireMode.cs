...
{
    public class BoardManagerQuickFireMode : MonoBehaviour
    {
        public GameManagerQuickFire gameManager;
        public QuickFireModeManager quickFireMode;
        ...
        public List<GameObject> pottedSpriteListBalls = new List<GameObject>();
        ...
        void Update()
        {
            ...
            if (gameManager.CurrentGameState == GameManagerQuickFire.GameState.isItCueballBreakDone)
            {
                //print("breake shot");
                ...
                if (isTurnCompleted)
                {
                    ...
                    CheckBreakShotRules();
                    pottedBallcount = pottedBalls.Count;
                    ...
                    {
                        gameManager.CurrentGameState = GameManagerQuickFire.GameState.isReadyForStrike;
                    }
            ...
            else if (gameManager.CurrentGameState == GameManagerQuickFire.GameState.isStrikeDone)
            {
                bool isTurnCompleted = true;
                for (int i = 0; i < remainingBallsInPlay.Count; i++)
                {
                ...
                if (isTurnCompleted)
                {
                    ...
                    {
                        gameManager.CurrentGameState = GameManagerQuickFire.GameState.isGameOver;
        ...
        public void PocketedBallDetails(Collision collision)
        {
            switch (collision.collider.tag)
            {
                case "CueBall":
                    cueBallPotted = true;
                    //quickFireMode.CueBallPottedFunction();
                    break;

                case "Ball":
                    {
                        ballPotted = true;
                        quickFireMode.BallPottedFunction();
                       // print("collision" + collision.collider.name);
                        ...
                        if (!collision.gameObject.name.Contains("8Ball"))
                        {
                           ...
                           // print("pottedBalls" + pottedBalls.Count);
                        }
                        if (gameManager.CurrentGameState != GameManagerQuickFire.GameState.isItCueballBreakDone)
                        {
        ...
        //Remove the velocity of potted ball and then send it to the tray immediately
        ...
        private void SendBallToTray(GameObject ball)
        {
            ...
            ball.GetComponent<BallCollisionHandlerQuickFireMode>().StartTrayMovementForPottedBall(target);
            pottedBallcounter++;
        }
