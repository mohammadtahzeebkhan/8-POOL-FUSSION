...
{
    public class BallCollisionHandlerLuckyShot : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (this.rb.linearVelocity.magnitude >= 0.01f)
            {
                //Debug.DrawRay(transform.position, this.rb.velocity, Color.red);
                transform.RotateAround(
                    transform.position,
                    Vector3.Cross(this.transform.position, this.rb.linearVelocity.normalized),
                    this.rb.linearVelocity.magnitude * 450f * Time.deltaTime);
                ballHit = true;
               // print("start ball");
                //TRY
            ...
            if (ballHit == true)
            {
                ...
                {
                   // print("stop ball");
                    luckyShotManager.LuckyShotPrizeDetailsUpdate();
                }
            }
            else 
            {
                luckyShotManager.BollNotHitUpdateDetails();
            }
        ...
        public void OnCollisionEnter(Collision collision)
        {
            ...
            else if ((gameManager.CurrentGameState == GameManagerLuckyShot.GameState.isItCueballBreak || gameManager.CurrentGameState == GameManagerLuckyShot.GameState.isItCueballBreakDone))
            {
                ...
                {
                    secondVel = collision.gameObject.GetComponent<CueBallCollisionHandlerLuckyShot>().VelocityBeforePhysicsUpdate;
                }
                else
                {
                    secondVel = collision.gameObject.GetComponent<BallCollisionHandlerLuckyShot>().VelocityBeforePhysicsUpdate;
                }
                if (collision.gameObject.GetComponent<Rigidbody>().linearVelocity.magnitude > secondVel.magnitude)
                {
                    secondVel = collision.gameObject.GetComponent<Rigidbody>().linearVelocity;
                }
                ...
                if (cueStickManager.FirstHitDoneCueBall)
                {
                    if (collision.gameObject.tag == "CueBall")
                    {
                        ...
                        this.rb.linearVelocity = new Vector3(0.008f, 0.008f, 0.008f);
                        collision.gameObject.GetComponent<Rigidbody>().linearVelocity = new Vector3(0.008f, 0.008f, 0.008f);
                        //this.rb.angularVelocity = Vector3.zero;
                        //collision.gameObject.GetComponent<Rigidbody>().angularVelocity = Vector3.zero;
