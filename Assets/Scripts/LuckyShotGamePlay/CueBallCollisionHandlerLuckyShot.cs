        ...
        public Vector3 VelocityBeforePhysicsUpdate { get => velocityBeforePhysicsUpdate; set => velocityBeforePhysicsUpdate = value; }
            ...
            if (this.rb.linearVelocity.magnitude >= 0.05f)
            {
                transform.RotateAround(transform.position,
                    ...
                    this.rb.linearVelocity.magnitude * spinValue * Time.deltaTime);
                cueBallMove = true;
            ...
            if (this.rb.linearVelocity.magnitude <= 0.05f && cueBallMove ==true)
            {
                cueBallStop = true;
            ...
            private void OnCollisionEnter(Collision collision)
            {
                ...
                if (collision.collider.tag == "BoardSides")
                {
                    ...
                    if (cueStickManager.FirstHitDoneCueBall)
                    {
                        ...
                        {
                            this.rb.linearVelocity = Quaternion.AngleAxis(cueStickManager.CueSpinAngle / 6, cueStickManager.cueStick.up) * this.rb.linearVelocity;
                        ...
                        {
                            //print("Top");
                            this.rb.linearVelocity = this.rb.linearVelocity * 1.15f;
                        }
                        else if ((cueStickManager.TorqueZValue == -1 && cueStickManager.TorqueXValue == -1) ||
                                (cueStickManager.TorqueZValue == 1 && cueStickManager.TorqueXValue == -1))//Back
                        {
                            //print("Back");
                            this.rb.linearVelocity = this.rb.linearVelocity * 0.5f;
                ...
                else if (collision.collider.tag == "Ball")
                {
                    ...
                    {
                        //print("Top");
                        this.rb.linearVelocity = this.rb.linearVelocity * 1.2f;
                    }
                    else if ((cueStickManager.TorqueZValue == -1 && cueStickManager.TorqueXValue == -1) ||
                            (cueStickManager.TorqueZValue == 1 && cueStickManager.TorqueXValue == -1))//Back
                    {
                ...
                if (hudManager.IsCueBallPressed && (collision.collider.tag == "Ball" || collision.collider.tag == "BoardSides"))
                {
                    ...
                    {
                        collision.collider.GetComponent<Rigidbody>().linearVelocity = Vector3.zero;
